...
const Banner3 = ({ title, description, cta, image, background, onEdit,editButton }: BannerProps
) => {
  ...
  return (
    <div className='relative w-full overflow-hidden opacity-100'>
      ...
      <div className='absolute inset-0 top-10 left-5'>
        <div className=' text-white text-3xl font-bold w-56'>
          {title}
        </div>
        <div className='absolute top-28 text-lg text-white w-72'>
          {description}
        </div>
        <div className='absolute top-56 border p-3 bg-white text-blue-500 font-semibold h-12 flex justify-center items-center rounded-lg'>
          {cta}
        </div>
      </div>
    </div>
